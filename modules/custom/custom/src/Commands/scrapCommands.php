 ...
 */
class scrapCommands extends DrushCommands
{
/**
 * Scrap Products
 *
 * @command custom:scrap
 * @aliases scrap
 * @param string $site
 * @options array
 * @usage scrap
 */
public function runCommands($site,$options=['count'=>FALSE])
    {
      if($site == 'yaoota')
      {
        /**
...
$prodimage =$fileRepository->writeData($data, "public://products/");
...
$this->output()->writeln('New Products Added '.$counter);
...
exit;
  ...
  $this->output()->writeln('No Site Selected for scrapping Ex:[yaoota,souq,olx] ');
